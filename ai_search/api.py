...
def search_items(query: str, limit: int = 20):
    """Direct Meilisearch query returning JSON response."""
    _check_meili_config()
    try:
        resp = requests.post(
        ...
        frappe.log_error(frappe.get_traceback(), title=f"ai_search.search_items error: {e}")
        return {"hits": []}

@frappe.whitelist()
def setup_index():
    """Set sensible Meilisearch index settings."""
    _check_meili_config()
    settings = {
...
def link_search_items(doctype: str, txt: str, searchfield: str, start: int, page_len: int, filters=None):
    """Server-side method used by Link field search.
    Returns a list of [value, description] pairs.
    Uses Meilisearch if configured; falls back to DB token search.
    """
    ...
    if _is_meili_configured():
        try:
            # Meilisearch handles tokenization/typos internally
            resp = requests.post(
                f"{MEILI_URL}/indexes/{INDEX}/search",
                headers=_headers(),
            ...
            frappe.log_error(frappe.get_traceback(), title=f"ai_search.link_search_items Meili error: {e}")
            # fall through to DB search

    # Fallback: token AND search on DB
    conds = []
    params = []
    for t in tokens:
